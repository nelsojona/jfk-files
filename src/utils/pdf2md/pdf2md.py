...
# Initialize client lazily to avoid prompting during import
openai = None

def get_openai_client():
    """Get OpenAI client, initializing it if necessary"""
    global openai
    if openai is None:
        # Check if running in CI environment
        is_ci = os.environ.get('CI') == 'true' or os.environ.get('GITHUB_ACTIONS') == 'true'
        
        # In CI, use a dummy key if real one isn't available
        if is_ci and not os.environ.get('OPENAI_API_KEY'):
            api_key = 'dummy-key-for-ci'
        else:
            # For normal operation, get key from env or prompt
            api_key = os.environ.get('OPENAI_API_KEY')
            if not api_key:
                try:
                    api_key = getpass(prompt='OpenAI API key:')
                except (EOFError, OSError):
                    # Handle case where stdin is redirected and prompting fails
                    print("Error: OPENAI_API_KEY environment variable not set and unable to prompt for key.", file=sys.stderr)
                    print("Please set the OPENAI_API_KEY environment variable.", file=sys.stderr)
                    api_key = 'dummy-key-missing-real-key'  # Allow import to continue
